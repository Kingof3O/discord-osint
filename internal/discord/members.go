	...
	if query != "" {
		...
		if err == nil {
			for _, m := range res {
				...
				avatarURL := ""
				if m.Avatar != "" {
					avatarURL = fmt.Sprintf("https://cdn.discordapp.com/guilds/%s/users/%s/avatars/%s.png", guildID, m.User.ID, m.Avatar)
				} else if m.User.Avatar != "" {
					avatarURL = fmt.Sprintf("https://cdn.discordapp.com/avatars/%s/%s.png", m.User.ID, m.User.Avatar)
				}
