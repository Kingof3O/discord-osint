...
```mermaid
graph TD
    subgraph Core["Workflow Engine and Lifecycle"]
        E["Engine Orchestrator"]
        R["Rate Limiter and Jitter"]
        S["Pure-Go SQLite Store"]
    end

    subgraph Adapters["Protocol and External Adapters"]
        D["Discord REST and Gateway"]
        C["CAPTCHA Local Bridge"]
        O["Onboarding and Gate Handler"]
        B["Disboard Scraper"]
    end

    subgraph Targets["Target Analysis and Matching"]
        V["Snowflake Preflight Engine"]
        M["Multi-Factor Identity Matcher"]
    end

    subgraph Presentation["Exporters and Interfaces"]
...
- **Member Discovery Tiers**:
  - **Tier 0**: Queries `GET /guilds/:guild_id/members/:target_id` to retrieve authoritative avatar CDN hashes (`/avatars/:id/:hash.png`), server nicknames, and assigned role IDs.
...
sequenceDiagram
    ...
    CLI->>Bridge: Launch with SiteKey, RqData, SessionID
    ...
    Browser->>Bridge: POST /submit with solution token
    Bridge-->>CLI: Return solution token and session ID
    CLI->>Disc: Re-submit with X-Captcha-Session-Id and captcha_key
...
1. **Membership Screening Rules**:
   Inspects `member.Pending`. If `true`, queries screening configuration and immediately submits `PUT /guilds/:guild_id/requests/@me`.
2. **Channel Reactions**:
   Scans welcome/rules channels for verification prompts with checkmarks, emulating `PUT /channels/:chan_id/messages/:msg_id/reactions/checkmark/@me`.
3. **Component Buttons**:
...
```mermaid
graph TD
    INPUT["Observed Member or Message"] --> ID_CHECK{"Does Snowflake ID match Target User ID?"}
    
    ID_CHECK -->|Yes| CONFIRMED["Confirmed: user_id_exact"]
    ID_CHECK -->|Author Hit| CONFIRMED2["Confirmed: message_author_id_exact"]
    
    ID_CHECK -->|No or Unknown| UNAME_CHECK{"Does Normalized Username match Target?"}
    
    UNAME_CHECK -->|Yes| CANDIDATE1["Candidate: username_exact"]
    UNAME_CHECK -->|No| NICK_CHECK{"Levenshtein Nickname Similarity at least 0.85?"}
